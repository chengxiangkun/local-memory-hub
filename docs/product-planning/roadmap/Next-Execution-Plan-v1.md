...
4. ✅ P1.1 embedding 接入和向量重建（已完成：可拔插模型目录 + 本地 Transformers.js 模型 + 下载/配置引导 + 向量重建；默认 multilingual-e5-small 本地免费方案，已替代 local_weak）。
5. ✅ P1.3 源资料详情页（已完成：详情抽屉、重新解析、片段级隔离、计数与追溯）。
6. ✅ P2.1 内嵌 SQLite（已完成：better-sqlite3 替换 sqlite3 CLI,根治高维向量缓冲,48 调用方零改动,27 组测试全绿）。
7. ✅ P2.3 正式图谱渲染器（已完成：vendored force-graph，Canvas 力导向适配器替换 SVG，保留模式/选中/搜索/缩放）。
8. ✅ P3.1 飞书增量/修改/删除轮询同步（已完成：edit_time 增量、删除标记、token 缓存+节流;腾讯/有道仍需各自凭证）。
9. ✅ P3.2 正式 MCP（stdio,官方 SDK）+ 工具权限与调用审计 UI（已完成）。
10. ✅ P2.2 Tauri 桌面运行时脚手架（已完成:sidecar 模式,cargo build 编译通过;打包安装器需 Apple 签名）。
...
V1 路线图的全部主项已完成:P0.1/P0.2/P0.3/P1.1/P1.3/P2.1/P2.2/P2.3/P3.1/P3.2
(连同 provider 修复),均含回归测试并已推送 GitHub。测试套件 28 个测试组全绿。
