...
- **P2.1 内嵌 SQLite**：`better-sqlite3` 原生依赖需联网与本地编译,且替换核心存储有数据损坏风险,建议单独排期并做迁移演练。注:高维向量(e5 384/1024 维)的 sqlite CLI 缓冲问题已临时用 maxBuffer 调大缓解,内嵌 SQLite 是根治方案。
- **P2.2 桌面运行时（Tauri）**：Rust 工具链已安装(rustc 1.96.0),可继续 Tauri 脚手架与编译验证。
- **P3.1 真实外部文档同步**：飞书凭证已持久化到 `.env.local`(gitignore 排除)并验证有效,端到端真实导入还需用户提供一个测试文档/文件夹 URL。腾讯文档/有道仍待接入。
- **P1.1 embedding**：已完成(默认本地 multilingual-e5-small)。
