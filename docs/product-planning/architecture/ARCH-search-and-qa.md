...
原因是本地记忆长期增长后，全量图会变成性能和认知负担。搜索子图让用户先看到可解释的局部关系，再通过点击节点继续展开。
...
如果只依赖单一路向量，相同含义的中英文表达可能落在不同区域；如果只做关键词，中文连续文本又容易被错误切分。因此 V1 检索需要采用混合策略：
...
- `vector_score` 负责语义近似。
- `lexical_score` 负责中英文混合、缩写、专有名词和精确主题校准。
- 对中文使用 2-gram，比单字更稳定，也比整句 token 更不容易偏移。
- 对英文保留长度大于等于 2 的词，过滤基础停用词。
- 后续如果引入真实 embedding 模型，应优先选择多语言模型或中英双语表现稳定的本地模型，并保留 lexical rerank。
